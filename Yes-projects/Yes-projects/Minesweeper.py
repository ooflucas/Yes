...
def find_adjacent_cells(positions):
    ...
    if positions == top_right:
        left, lower, lower_left = positions - 1, positions + column, positions + column - 1
        adjacent_list = [lower, left, lower_left]
        return adjacent_list
    elif positions == top_left:
        right, lower, lower_right = positions + 1, positions + column, positions + column + 1
        adjacent_list = [right, lower, lower_right]
        return adjacent_list
    elif positions == bottom_left:
        upper, upper_right, right = positions - column, positions - column + 1, positions + 1
        adjacent_list = [upper, upper_right, right]
        return adjacent_list
    elif positions == bottom_right:
        left, upper, upper_left = positions - 1, positions - column, positions - column - 1
        adjacent_list = [left, upper, upper_left]
        return adjacent_list
    elif positions in topside:
        lower, left, right, lower_left, lower_right = positions + column, positions - 1, positions + 1, positions + column - 1, positions + column + 1
        adjacent_list = [lower, left, right, lower_left, lower_right]
        return adjacent_list
    elif positions in bottomside:
        upper, left, right, upper_left, upper_right = positions - column, positions - 1, positions + 1, positions - column - 1, positions - column + 1
        adjacent_list = [upper, left, right, upper_left, upper_right]
        return adjacent_list
    elif positions in leftside:
        upper, lower, right, upper_right, lower_right = positions - column, positions + column, positions + 1, positions - column + 1, positions + column + 1
        adjacent_list = [upper, lower, right, upper_right, lower_right]
        return adjacent_list
    elif positions in rightside:
        upper, lower, left, upper_left, lower_left = positions - column, positions + column, positions - 1, positions - column - 1, positions + column - 1
        adjacent_list = [upper, lower, left, upper_left, lower_left]
        return adjacent_list
    else:
        upper, lower, left, right, upper_left, upper_right, lower_left, lower_right = positions - column, positions + column, positions - 1, positions + 1, positions - column - 1, positions - column + 1, positions + column - 1, positions + column + 1
